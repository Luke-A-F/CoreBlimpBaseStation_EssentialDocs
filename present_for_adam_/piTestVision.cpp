...
#define G_MIN_CONFIDENCE         0.1
#define G_POLY_APPROX_E          0.001
#define GOAL_INNER_CONTOUR_SCALE 0.4

//COLORS
#define COLOR_SPACE COLOR_BGR2HSV

//balloon
#define B_MIN           Scalar(37,25,0)
#define B_MAX           Scalar(86,84,255)
#define B_Correction    Scalar(23,0,16)

//yellow goal
#define GY_MIN          Scalar(0,0,0)
#define GY_MAX          Scalar(98,255,255)
#define GY_Correction   Scalar(43,0,0)

//orange goal
#define GO_MIN          Scalar(0,0,0)
#define GO_MAX          Scalar(19,255,255)
#define GO_Correction   Scalar(17,32,0)
...
pthread_t tVision;
long tVisionID;
pthread_mutex_t visionMutex;	//vision mutex

void *visionThread(void *threadID) {
	fprintf(stdout, "Thread (%d) successfully started.\n", *((int*)(&threadID)));
...
int main(int argc, char** argv) {
	...
	int visionRC = pthread_create(&tVision, NULL, visionThread, (void *)tVisionID);
	if (visionRC != 0) {
		fprintf(stdout, "pthread_create() failed with return code %d\n", visionRC);
	}

	while(true) {
		clock_t currentTime = clock();
		double cycleTime = (currentTime - lastCycle)/CLOCKS_PER_SEC;
...
VideoCapture openCamera(int camIndex, int camWidth, int camHeight){
	...
	struct v4l2_control c_exposure;
	c_exposure.id = V4L2_CID_EXPOSURE_AUTO;
	c_exposure.value = 3; // 1=manual, 3=auto; V4L2_EXPOSURE_AUTO fails...
	if (v4l2_ioctl(device, VIDIOC_S_CTRL, &c_exposure) != 0) {
